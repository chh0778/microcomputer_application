...
// Clock Enable : GPIOD & TIMER4
	RCC->AHB1ENR	|= (1<<_);	// GPIOD Enable
	RCC->APB1ENR 	|= (1<<_);	// TIMER4 Enable 

// PD12: TIM4_CH1
// PD12을 출력설정하고 Alternate function(TIM4_CH1)으로 사용 선언
	GPIOD->MODER 	|= (__<<__);	// GPIOD PIN12 Output Alternate function mode 					
	GPIOD->OSPEEDR 	|= (__<<__);	// GPIOD PIN12 Output speed (50MHz High speed)
	GPIOD->PUPDR	&= ~(__<<__); 	// GPIOD PIN12 NO Pull-up
  					// PD12 ==> TIM4_CH1
	GPIOD->AFR[1]	|= (__<<__);	// Connect TIM4 pins(PD12) to AF2(TIM3..5)
 
// PD13: TIM4_CH2
// PD13을 출력설정하고 Alternate function(TIM4_CH2)으로 사용 선언
	GPIOD->MODER 	|= (__<<__);	// GPIOD PIN13 Output Alternate function mode 					
	GPIOD->OSPEEDR 	|= (__<<__);	// GPIOD PIN13 Output speed (50MHz High speed)
	GPIOD->PUPDR	&= ~(__<<__); 	// GPIOD PIN13 NO Pull-up
  					// PD13 ==> TIM4_CH2
	GPIOD->AFR[1]	|= (__<<__);	// Connect TIM4 pins(PD13) to AF2(TIM3..5)
...
// Timerbase Mode
	...
	TIM4->CR1 &= ~(3<<5); 	// CMS(Center-aligned mode Sel)=00 (Edge-aligned mode) (reset state)
        ...
        // CR2 
        TIM4->CR2 &= ~(__<<__);	// TI1S=0(TIM4_CH1 pin is connected to TI1 input)
 
	// PSC, ARR
	TIM4->PSC = 1-1;		// Prescaler=1
	TIM4->ARR = _____;	// Auto reload  :  엔코더 count값 범위: 0~10000
...
// Output Compare Mode
	...
	TIM4->CCMR1 |= (__<<__); 	// CC1S(CC1 channel) = '0b01' : Input 
	TIM4->CCMR1 &= ~(15<<4);   // IC1F='0b0000: No Input Filter 

    	TIM4->CCMR1 |= (__<<__); 	// CC2S(CC2 channel) = '0b01' : Input 
	...
	TIM4->CCER &= ~(1<<__);	// CC1P=0: TI1FP1 NonInverting / Rising Edge   
	TIM4->CCER &= ~(1<<__);	// CC1NP=0:   

      	TIM4->CCER &= ~(1<<__);	// CC2P=0: TI2FP1 NonInverting / Rising Edge   
	TIM4->CCER &= ~(1<<__);	// CC2NP=0:   

        
	// SMCR(Slave Mode Control Reg.) : Encoder Mode  3 
	TIM4->SMCR |= (__<<__);	// SMS(Slave Mode Selection) : Encoder mode 1
