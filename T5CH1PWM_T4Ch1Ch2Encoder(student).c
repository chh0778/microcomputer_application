...
// PD13을 출력설정하고 Alternate function(TIM4_CH2)으로 사용 선언
	...
	GPIOD->PUPDR	&= ~(3<<26); 	// GPIOD PIN13 NO Pull-up
  					// PD13 ==> TIM4_CH2
	GPIOD->AFR[1]	|= (2<<20);	// Connect TIM4 pins(PD13) to AF2(TIM3..5)              //우선 이거 틀렸음
...
// Timerbase Mode
	...
	TIM4->CR1 &= ~(3<<5); 	// CMS(Center-aligned mode Sel)=00 (Edge-aligned mode) (reset state)
        ...
        // CR2 
        TIM4->CR2 &= ~(1<<7);	// TI1S=0(TIM4_CH1 pin is connected to TI1 input)       //이것도 틀렸음
...
// Output Compare Mode
	...
	TIM4->CCER &= ~(1<<3);	// CC1NP=0:   
      	...
      	TIM4->CCER &= ~(1<<5);	// CC2P=0: TI2FP1 NonInverting / Rising Edge   
	TIM4->CCER &= ~(1<<7);	// CC2NP=0:   

        
	// SMCR(Slave Mode Control Reg.) : Encoder Mode  1
	TIM4->SMCR |= (1<<0);	// SMS(Slave Mode Selection) : Encoder mode 1
...
void TIMER2_Init(void)
{
    ...
    // Deciding the Period
	TIM2->PSC = 84-1;	// Prescaler 84,000,000Hz/84 = 1,000,000 Hz (1Ms)  (1~65536)
	TIM2->ARR = 1000000;	// Auto reload  1Ms * 1,000,000 = 1s
...
{	TIM2->SR &= ~(1<<0);	// Interrupt flag Clear
	RPM = TIM4->CNT;		// TIM4 CNT 값(Encoder 펄스 수) 읽음
	TIM4->CNT = 0;
	RPM = RPM*60 /3;
